...
#
# Vídeo de instalação do GNU/Linux Ubuntu Server 22.04.x LTS: https://www.youtube.com/watch?v=zDdCrqNhIXI
# Vídeo de instalação do LAMP Server no GNU/Linux Ubuntu Server 22.04.x LTS: https://www.youtube.com/watch?v=6EFUu-I3u4s
#
...
#
# Verificando se o usuário é Root, Distribuição é >=22.04 e o Kernel é >=4.15 <IF MELHORADO)
# [ ] = teste de expressão, && = operador lógico AND, == comparação de string, exit 1 = A maioria dos erros comuns na execução
clear
if [ "$USUARIO" >= "0" ] && [ "$UBUNTU" >= "22.04" ] && [ "$KERNEL" >= "4.15" ]
	then
		...
		echo -e "Usuário não é Root ($USUARIO) ou Distribuição não é >=22.04.x ($UBUNTU) ou Kernel não é >=4.15 ($KERNEL)"
...
echo -n "Verificando as dependências ZoneMinder, aguarde... "
	...
	do
		...
		sleep 5
#		
# Script de instalação do ZoneMinder no GNU/Linux Ubuntu Server 22.04.x
